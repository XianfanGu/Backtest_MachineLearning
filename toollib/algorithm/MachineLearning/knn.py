...
def initialize(context):
    ...
    context.model = KNeighborsClassifier(n_neighbors=feature_num, p=3, metric='minkowski')  # 7.05 for  model
...
try:
    for ele in SnPList:
        SYMBOL = ele
        if(os.path.isfile(('output/'+SYMBOL+'_KNN_output.csv'))):
            print('output/'+SYMBOL+'_KNN_output.csv is exist')
            continue
        else:
            print('output/' + SYMBOL + '_KNN_output.csv is not exist')
        ...
        # Print
        perf_manual.to_csv('output/'+SYMBOL+'_KNN_output.csv')
except Exception as error:
    pass
